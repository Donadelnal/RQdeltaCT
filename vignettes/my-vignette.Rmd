...
#### 2. Reading wide-format data using the `read_Ct_wide()` function.
...
* `RQ_exp_Ct_dCt()` - this function performs relative quantification by:  
  ...
  + statistical testing of differences in transformed Ct values between the study group and the reference group. Student's t test and Mann-Whitney U test are implemented and resulting statistics (in column with the "_test_stat" pattern) and p values (in column with the "_test_p" pattern) are returned.
...
The `RQdeltaCT` package includes `find_ref_gene()` function that can be used to select the best reference gene for normalisation. This function calculates descriptive statistics such as minimum, maximum, standard deviation, and variance, as well as more advanced parameters including the colinearity coefficient VIF (separately for each group) and stability scores calculated using the [NormFinder](https://www.moma.dk/software/normfinder) ([Article](https://aacrjournals.org/cancerres/article/64/15/5245/511517/Normalization-of-Real-Time-Quantitative-Reverse)) and [geNorm](https://genorm.cmgg.be/) ([Article](https://genomebiology.biomedcentral.com/articles/10.1186/gb-2002-3-7-research0034)) algorithms. Ct values are also presented on a line plot.  

The VIF colinearity coefficient is calculated using the `car::vif()` function running on multiple linear regression models with all genes as predictory variables and a random dummy response variable. NormFinder scores are computed using internal `RQdeltaCT::norm_finder()` function working on the code adapted from the original NormFinder code. For the geNorm score, the `geNorm()` function of the `ctrlGene` package is used.

Due to different resistance to data properties, the `find_ref_gene()` function allows one to choose which of these algorithms should be done by setting the logical parameters: `vif.score`, `norm.finder.score`, and `genorm.score`. 

* The VIF score is the most sensitive: it could be calculated for only two groups of samples, missing data could interfere eith the correlation results (e.g., warning "In cov2cor(v) : diag(.) had 0 or NA entries; non-finite result is doubtful" can appear), and in the case of perfect correlation between genes, the error "Error in vif.default(model) : there are aliased coefficients in the model" will be displayed and the process will stop (it could especially occur when a low number of samples is analysed). In such a situation, `vif.score` should be set to FALSE.
* The NormFinder algorithm is calculated on data with missing values and more than three groups, but at least two samples must be in each group.
* geNorm is free from the above limitations.
...
**NOTE:** Missing data can be present in the data, but it could generate a warning: "diag(.) had 0 or NA entries; non-finite result is doubtful" returned from the `vif()` function.  
...
**NOTE:** Minimum of three samples or genes in the data is required for these analyses. In PCA analysis, samples with missing values will be removed before computation.
