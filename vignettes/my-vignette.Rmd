...
Firstly, the `results_boxplot()` function should be run with `signif.show = FALSE` to avoid drawing labels, and then a data frames with coordinates for statistical significance labels are added for each pair (left, right, edges) of distributions: 
...
Three data frames with specified coordinates for each of three pairs of distributions are subsequently prepared:
...
And finally, the plot with customized localization of labels can be drawn using three prepared data frames:
