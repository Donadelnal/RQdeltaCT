...
* [**Analysis of more than two groups**]
  ...
  + [Quality control and filtering of transformed Ct data - a multigroup variant of analysis]
...
2. 2^-ddCt^ method. Similarly to the 2^-dCt^ method, Ct values are normalised by endogenous control gene, but the obtained delta Ct (dCt) values are not exponentially transformed, but summarised by means in the compared study groups, and the mean dCt in a control group is subtracted from the mean dCt in a study group, giving the delta delta Ct (ddCt) value. Subsequently, the ddCt values are transformed using the 2^-ddCt^ formula to obtain the fold change value (also called the RQ value). This method is useful where a compared groups contain rather technical than biological replicates, e.g. where samples of cell line before adding stimulant are compared to samples of the same cell line after stimulation. See examples no. 1 and 2 in [Article2](https://www.nature.com/articles/nprot.2008.73).  
...
All datasets used in this chapter are a part of real data used in article [Zalewski, D.; Chmiel, P.; Kołodziej, P.; Borowski, G.; Feldo, M.; Kocki, J.; Bogucka-Kocka, A. Dysregulations of Key Regulators of Angiogenesis and Inflammation in Abdominal Aortic Aneurysm. Int. J. Mol. Sci. 2023, 24, 12087](https://doi.org/10.3390/ijms241512087). The tables contain samples divided into two groups: AAA group (patients with abdominal aortic aneurysm) and Control group (subjects without AAA). For each sample, the expression of 19 genes were determined as Ct values, obtained using real-time PCR method and TaqMan assays.
...
**NOTE:** These functions do not perform data filtering, but only report numbers of Ct values labelled as reliable or not and presents them graphically.  
...
To gain deeper insight into number if replicates by genes and samples, `control_heatmap()` function can be used:
...
**NOTE:** The `control_heatmap()` works only if various numbers of replicates are in the data, otherwise error will appear because inherited `pheatmap()` function can not deal with the situation where all values are equal.

Visual inspection of returned plots and obtained tables gives a clear image of data quality. The results obtained in the examples show that the AAA group contains more samples than the Control group. Some of the samples have more Ct values (more technical replicates) than other samples. Furthermore, in all samples, the majority of Ct values are reliable.  

Regarding genes, GAPDH and FGF23 were investigated in duplicates in the Control group, other genes have single Ct values in both groups. Furthermore, FGF23 was analysed only in the Control group and has all values labeled as unreliable; therefore, it is obvious that this gene should be excluded from the analysis. Some other genes also have many unreliable Ct values (e.g. ANGPT2, IL1A) and maybe should be considered to be removed from the data.  
...
Among tested genes, GAPDH, TNF, TGFB, and VEGFA seem to have the best characteristics to be a reference gene (they have low variance, low NormFinder and low geNorm scores).
...
final_boxplot_no_fac <- results_boxplot(data = data.dCtF,
                                 sel.Gene = c("ANGPT1","IL8", "VEGFB"),
                                 by.group = TRUE,
                                 signif.show = FALSE,  # Hide significance labels
...
Step 4: Some minor improvements and the plot is ready! 

```{r fig.dim=c(5,4.5), fig.align='center', cache=FALSE}
final_boxplot_no_fac_ok <- final_boxplot_no_fac_ok +
                            theme(axis.text.x = element_text(face="italic")) + # For human gene symbols, italic font was used
...
If genes should be displayed in the alphabetical order, simply sort them:
...
The `control_heatmap()` does not work here, because inherited `pheatmap()` function can not deal with the situation where all values are equal (in this example all values are equal to 2). 
...
data.dCt.pairwise.F.slim <- pivot_longer(data.dCt.pairwise.F, cols = Gene10:Gene8, names_to = "gene", values_to = "exp")

# Select genes
data.dCt.pairwise.F.slim.sel <- data.dCt.pairwise.F.slim[data.dCt.pairwise.F.slim$gene %in% c("Gene19","Gene8"), ]

# Change order of groups if needed
data.dCt.pairwise.F.slim.sel$Group <- factor(data.dCt.pairwise.F.slim.sel$Group, levels = c("Before","After"))
...
Many functions of the `RQdeltaCT` package work well with data containing more than two groups. To demonstrate this
functionality, an example of analysis regarding data with three groups is presented below 
(variant with comparison of independent groups). For the purpose of this analysis, 
the package dataset named `data.Ct.3groups` was used. Of course, data with multiple groups can be imported to R using options previously described in the chapter [**Data import**].
...
The `data.Ct.3groups` dataset contains a table with gene expression data obtained for 20 genes and 104 samples 
using qPCR experiments with TaqMan assays. The samples are divided into three groups: Disease AAA (40 samples), 
Disease VV (40 samples), and Control (24 samples). This dataset is a part of data used in articles: 
...
* [Zalewski, D.; Chmiel, P.; Kołodziej, P.; Kocki, M.; Feldo, M.; Kocki, J.; Bogucka-Kocka, A. Key Regulators of Angiogenesis and Inflammation Are Dysregulated in Patients with Varicose Veins. Int. J. Mol. Sci. 2024, 25, 6785](https://doi.org/10.3390/ijms25126785)

This table is a data frame with 2048 rows and 5 variables (Sample, Gene, Ct, Group, and FLAG). 

The workflow for multiple groups presented here is very similar to the variant for two groups presented in chapter [**Part A: The workflow for analysis of independent groups of samples**], therefore we will focus on the most important points and differences, and prior reading of this previous chapter is highly recommended.
...
On the heatmap we can clearly observe that some of the samples have more Ct values (more technical replicates) than other samples. Genes FGF23 and GAPDH were investigated in duplicates in the Control group, other genes have single Ct values. 
...
This step is also similar to that for two groups. You need to provide a list of group names in the `groups` argument. In the example below, five genes are tested for suitability to be a reference gene: 
...
In the scenario where unnormalised data should be analysed, the `normalise` parameter should be set to FALSE.
...
Currently, the `RQ_dCt()` and `RQ_ddCt()` functions work only with two groups (study group and reference group) that are specified in the ``group.study` and `group.ref` arguments. The functionalities for the analysis of more than two groups will be extended in the future.
...
Five functions:  `FCh_plot()`, `results_volcano()`, `results_barplot()`, `results_boxplot()`, and `results_heatmap()` were created for visualisation of results. Two of them, `FCh_plot()` and `results_volcano()`, process data returned from two group-specific `RQ_dCt()` and `RQ_ddCt()` functions, therefore analysis of data with more two groups is not applicable. 

The remaining three functions (`results_barplot()`, `results_boxplot()`, and `results_heatmap()`) can be used successfully for data with multiple groups, and the user must provide a vector of colors for groups. 

However, the `results_barplot()` and `results_boxplot()` functions are designed to draw the statistical significance labels for two groups (see [The `results_boxplot()` function] chapter). If distributions or bars of three (or more) groups are drawn, the labels will appear above all boxplots. This is not an optimal solution:
...
Step2: Data frames with coordinates for statistical significance labels for each pair (left, right, edges) of distributions are subsequently prepared:
...
Firstly, the `results_barplot()` function should be run with `signif.show = FALSE` to avoid drawing labels, and then data frames with coordinates for statistical significance labels are added for each pair (left, right, edges) of distributions: 
...
Principal component analysis (PCA) and k means clustering works well with multiple groups, but the number of colors for points must be equal to the number of groups.
...
The functions performing correlation analysis of samples (`corr_sample()` function) and genes (`corr_gene()` function) are group-independent and can be used for multigroup data.
...
**Remember:** A minimum of 5 samples/target are required for correlation analysis.

---

The function designed for simple linear regression analysis of samples (`single_pair_sample()`) is group independent. 
...
However, the function for simple linear regression analysis of genes (`single_pair_gene()`) is group-independent only in ungrouped mode (`by.group = FALSE`).
...
If the analysis is performed with stratification by groups, a vector with colors must be provided in `colors` arguments, and the position of labels should be adjusted.
...
The Receiver Operating Characteristic method works with only two groups, thus the `ROCh()` function requires also two groups, specified in `groups` argument.
...
Similarly to the Receiver Operating Characteristic method, logistic regression also works with only two groups, thus the `log_reg()` function requires also two groups, specified in `group.study` and `group.ref` arguments.
