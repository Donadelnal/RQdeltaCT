...
The `RQdeltaCT` package requires the following external packages: 
...
Ideally, the reference gene should have an identical expression level in all samples, but in many situations it in not possible to achieve, especially when biological replicates are analyzed. The lowest variance is desired, and it is also recommend that Ct values would not be very low (below 15) or very high (above 30) [Kozera and Rapacz 2013](https://link.springer.com/article/10.1007/s13353-013-0173-x).   
The `RQdeltaCT` package include `find_ref_gene()` function that could be useful to select best reference gene for normalization. It calculates descriptive statistics including minimum, maximum, standard deviation, and variance, as well as more advance parameters as colinearity coefficient VIF (separately for each group), and stability scores calculated using [NormFinder](https://www.moma.dk/software/normfinder) ([Article](https://aacrjournals.org/cancerres/article/64/15/5245/511517/Normalization-of-Real-Time-Quantitative-Reverse)) and [geNorm](https://genorm.cmgg.be/) ([Article](https://genomebiology.biomedcentral.com/articles/10.1186/gb-2002-3-7-research0034)) algorithms. Ct values are also presented on a line plot.  

VIF colinearity coefficient is calculated usig `car::vif()` function runing on multiple linear regression models with dummy response variable. NormFinder method was carried out using `RQdeltaCT::norm_finder()` working on a original NormFinder code. For geNorm score, `geNorm()` function from `ctrlGene` package was used. 
...
ref[[2]]
```
Among tested genes, Gene16 and Gene8 seems to have the best characteristics to be a reference gene (low variance, high VIF values, and low NormFinder and geNorm scores).
...
For visualisation of final results, three functions could be used `RQ_plot()`, `results_barplot`, and `results_boxplot()` illustrating fold change values, means with sd values and data distribution of targets, respectively. All these functions allow to add customized statistical significance labels to plots using `ggsignif` package.
...
On the barplot, bars of significant genes are distinguished by colors and/or labels. Significance of genes could be set by two criteria: p value and (optional) fold change, and thresholds for both criteria could be specified. `RQ_plot()` function offers various options of p values usage on the plot:
...
This function creates a barplot illustrating mean and sd values of data for selected target.

Data objects returned from `make_Ct_ready()`, `exp_Ct_dCt()` and `delta_Ct() functions` can be directly applied to this function.

```{r fig.dim=c(5,5), fig.align='center', cache=FALSE}
final_barplot <- results_barplot(data.dCtF,
...
library(Hmisc)
library(corrplot)
corr.samples <- corr_sample(data.dCtF[1:17, ],
                            order = "hclust")
```
```{r fig.dim=c(6.5,6.5), fig.align='center', cache=FALSE}
library(Hmisc)
