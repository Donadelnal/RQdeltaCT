...
```{r echo=FALSE, out.width="600px", dpi = 600, fig.align="center", warning=FALSE, message=FALSE, cache=FALSE}
library(here)
...
# Imported table is now transformed to long-format structure. X colimn is unnecessary and is removed. All variables are converted to character to unify class of variables.
library(tidyverse)
...
**NOTE:** Of course, data can be imported to R using user own code, but the final object must be data frame and contain table with column named "Sample" with sample names, column named "Target" with gene names, column named "Ct" with raw Ct values, column named "Group" with groups names, and optionally column named "Flag" that contain flag information (this column should be class of character or factor).
---
...
In some situations, unified fraction of unreliable data need to be establish and used to make decision which samples or genes should be excluded from the analysis. The `RQdeltaCT` package meets this needing and offers two additional functions: `low_quality_samples()` and `low_quality_genes()`. These functions return vector with samples or genes for which the amount of unreliable Ct values are higher than fraction specified in `fraction` parameter. The second element of object returned by  `control_Ct_barplot_sample()` and  `control_Ct_barplot_gene()` functions can be used directly:

```{r cache=FALSE}
low.quality.samples <- low_quality_samples(data = sample.Ct.control[[2]],
                                           fraction = 0.5)
low.quality.samples

# Remember to provide names of groups - only two groups can be specify:
low.quality.genes <- low_quality_genes(data = gene.Ct.control[[2]],
                                           fraction = 0.5,
...
**NOTE:** Functions `low_quality_samples()` and `low_quality_genes()` works with the second element (table) of lists returned from `control_Ct_barplot_sample()` and `control_Ct_barplot_gene()` functions, respectively.
...
**NOTE:** If data contain more than two groups, it is good practice to remove groups that are out of comparison; however, a majority further functions can deal with more groups unless two groups are indicated in function's parameter.
...
control.pca.sample <- control_pca_sample(data = data.dCt,
                                         ...
                                         label.size = 2.5)
```
```{r fig.dim=c(5.5,5), cache = TRUE}
control.pca.gene <- control_pca_gene(data = data.dCt)
```

**NOTE:** PCA algorithm can not deal with missing data (NAs); therefore, variables with NA values are removed before analysis. If at least one NA value occur in all variables in at least one of compared group, the analysis can not be done. Imputation of missing data will avoid this issue.
...
This function creates a barplot illustrating mean and sd values of data for all or selected genes.
...
All functions can be run on the entire data or only on selected samples/genes.
